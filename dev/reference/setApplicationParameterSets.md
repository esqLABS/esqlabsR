...
``` r
setApplicationParameterSets(project, id, parameterSets)
```
...
- id:
  ...
  Character vector of application ids. Each is canonicalized the same
  way
...
- parameterSets:
  ...
  Character vector of set ids (from `parameterSets` definitions),
  applied whole to every application; use `character(0)` to clear. To
  set a different list per application, pass a list of the same length
  as `id` (one character vector per application).
