server <- function(input, output, session){
  ...
  output$units <- renderUI({
    ...
    if (dimension %in% c("Concentration (mass)", "Concentration (molar)")){
      units <- c(ospsuite::ospUnits[["Concentration (mass)"]],
                 ospsuite::ospUnits[["Concentration (molar)"]])
      currentMolWeight <<- 1
    } else if (dimension %in% c("Amount", "Mass")){
      units <- c(ospsuite::ospUnits[["Amount"]],
    ...
    isolate({
      ui <- list(
        fluidRow(
          column(3, selectInput("unit1", "Unit 1", choices = units, selected = currentUnit1),
  ...
  observer_left <- observe({
    if(is.null(input$value1) || is.na(input$value1)) return()

    currentMolWeight <<- input$molweight
    currentUnit1 <<- input$unit1
    currentUnit2 <<- input$unit2

    converted <- toUnit(dimension, input$value1, targetUnit = currentUnit2,
  ...
  observer_right <- observe({
    if(is.null(input$value2) || is.na(input$value2)) return()

    converted <- toUnit(dimension, input$value2, targetUnit = currentUnit1,
