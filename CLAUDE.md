...
Descriptive only. No renames are implied or scheduled by this section; see issue [#1226](https://github.com/esqLABS/esqlabsR/issues/1226) for the architecture audit that identified these overlaps.
...
| PI member id | The `id` of a `PIParameter` or `PIOutputMapping`: a label unique within its own task, not a definition-file id. Every door stores it exactly as given and every lookup matches it exactly, so it is never canonicalized — unlike a PI *task* id, which names a file and is. | `id` field of `PIParameter()` / `PIOutputMapping()`; matched in `.removePIMember()` and the `add*` collision checks, all in `R/parameter-identification.R` |
