...
test_that("It can calculate the geometric mean and standard deviation", {
  meanDataSet <- calculateMeans(list(dataSet1, dataSet2), method = "geometric")
  expect_equal(meanDataSet$xValues, 1:6)
...
test_that("It can convert values to outputXunit and outputYunit", {
  ...
  meanDataSet <- calculateMeans(list(dataSet1, dataSet2), outputXunit = "min", outputYunit = "µg/l")
...
test_that("It can convert values to xUnit and yUnit of first data set, with given molWeights", {
  ...
  dataSet2$xUnit <- "min"
  meanDataSet <- calculateMeans(list(dataSet1, dataSet2), outputMolWeight = 5)
...
test_that("It throws an error when xValues can not be converted to same xUnit", {
  ...
  expect_error(calculateMeans(list(dataSet1, dataSet2)), "Unit 'l/min' is not defined in dimension 'Time'")
})
...
test_that("It throws an error when yValues can not be converted to same yUnit", {
  ...
  expect_error(
    calculateMeans(list(dataSet1, dataSet2)),
...
test_that("It throws an error when molWeights of data sets are different and no outputMolWeight is given", {
  ...
  expect_error(calculateMeans(list(dataSet1, dataSet2)), messages$errorOutputMolWeightNeeded())
})
...
test_that("It can handle the treatLLOQ argument", {
  ...
  meanDataSet <- calculateMeans(list(dataSet1, dataSet2), treatLLOQ = "LLOQ/2")
  expect_equal(meanDataSet$xValues, 1:6)
  expect_equal(meanDataSet$yValues, c(1, 3:6, 8), tolerance = 1e-06)
  expect_equal(meanDataSet$yErrorValues, c(NaN, rep(sqrt(2), 4), NaN), tolerance = 1e-06)
  # LLOQ
  meanDataSet <- calculateMeans(list(dataSet1, dataSet2), treatLLOQ = "LLOQ")
  expect_equal(meanDataSet$xValues, 1:6)
  ...
  # ZERO
  meanDataSet <- calculateMeans(list(dataSet1, dataSet2), treatLLOQ = "ZERO")
  expect_equal(meanDataSet$xValues, 1:6)
  ...
  # ignore
  meanDataSet <- calculateMeans(list(dataSet1, dataSet2), treatLLOQ = "ignore")
  expect_equal(meanDataSet$xValues, 1:6)
