...
- 2026-07-24 14:19: dataCombined validation now runs on the JSON spec directly via a `dataCombined` validation adapter (removed the legacy DataFrame reshape and the `FromExcel` validator), and `addDataCombined()` simulated entries accept an output-path id for `path` (resolved to the literal path when the DataCombined is built). Moved dataCombined shape validation out of the plots adapter so an error is no longer reported twice, and dropping a missing-dataSet observed entry now also skips its transform. (#1026, #1042)
