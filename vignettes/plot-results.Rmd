...
Once all the excel sheets are setup, just call the `createPlotsFromExcel()` function to generate the plots defined in the Excel file.
...
First, the user needs to define the name of the dataCombined. The name needs to
be stored in the `DataCombinedName` column. Then, for each `DataCombinedName`, the user
...
explored in objects created from the function `createEsqlabsPlotConfiguration()`.

Here is a sample of the available plot settings:

```{r, echo=FALSE}
dpc <- createEsqlabsPlotConfiguration()
names(dpc)
```
...
Then, the `Plot.xlsx` file is setup to define the `DataCombined`, customize the plots and specify the plotGrids. The `Plot.xlsx` file used in this example can be found
...
#### Save plots as PNG (or other file type)

The generated plots are of type `ggplot` and can be saved using the `ospsuite.plots::exportPlot`or the `ggplot2::ggsave()` function. Consider using the following code to save the generated plots in a `Figures` folder in the project directory:
...
exportPlotsToPNG <- function(
  ...
  if (!dir.exists(paths = outputPath)) {
    dir.create(path = outputPath, recursive = TRUE)
  }
...
The plotting functions return standard `ggplot2` objects. Customization —
specifying titles, axis labels, axis limits, theme settings, etc. — can be
applied using the standard `ggplot2` `+` operator (e.g., `+ ggplot2::labs(title = ...)`)
or via `ospsuite.plots::setDefaults()` for workspace-wide defaults.
...
projects, create the grid and export configurations using:
...
indivPlot <- plotTimeProfile(dataCombined) +
  ggplot2::labs(title = "Time profile")

obsVsSimPlot <- plotPredictedVsObserved(dataCombined) +
  ggplot2::labs(title = "Predicted-vs-observed")

resVsTimePlot <- plotResidualsVsCovariate(dataCombined, xAxis = "time") +
  ggplot2::labs(title = "Res-vs-time")

resVsSimPlot <- plotResidualsVsCovariate(dataCombined, xAxis = "predicted") +
  ggplot2::labs(title = "Res-vs-predicted")
