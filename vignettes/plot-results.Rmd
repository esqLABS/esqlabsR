...
function to generate the plots defined in the Excel file.
...
the plots and specify the plotGrids. The `Plot.xlsx` file used in this
...
The generated plots are of type `ggplot` and can be saved using the `ospsuite.plots::exportPlot`of the `ggplot2::ggsave()` function. Consider using the following code to save the generated plots in a `Figures` folder in the project directory:
...
exportPlotsToPNG <- function(
  ...
  outputPath <- file.path(getwd(), "Figures")
