...
the figures that need to be created. Then, `createPlots()` will
...
Once all the excel sheets are setup, just call the `createPlots()`
...
# Observed data is auto-loaded from the JSON configuration
observedData <- projectConfiguration$observedData
```
...
[here](https://github.com/esqLABS/esqlabsR/raw/main/inst/extdata/projects/Example/Configurations/Plots.xlsx).
...
```{r}
# Create DataCombined used in the plots "Aciclovir" and "Aciclovir2"
...
sheet with the function `createDataCombined()`, read (this
...
section of the `ProjectConfiguration.json` file and loaded automatically
...
# Observed data is automatically loaded from the JSON configuration
observedData <- projectConfiguration$observedData
