...
validation_results <- validateAllConfigurations(exampleProjectPath())
...
project$modelParametersFile <- "absolute/path/to/anotherModelParameters.xlsx"
```
...
A `Project.xlsx` file is also present for backwards compatibility
and can be used with `importProjectFromExcel()` to generate or
...
`Project.xlsx` file with the name `Project.json`.
