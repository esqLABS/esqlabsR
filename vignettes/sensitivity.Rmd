...
%, whereas ten-fold lower (−0.97) increases it by 2.13 %. The sensitivity
...
The results of a sensitivity analysis can be saved using `saveSensitivityCalculation()` 
and restored with `loadSensitivityCalculation()`. 
The `simulation` argument in `loadSensitivityCalculation()` is optional. If the 
original `Simulation` object is provided, reloading from disk is avoided. Otherwise, 
the function attempts to load the `.pkml` file from the path recorded during the 
original analysis, which must still be accessible.
