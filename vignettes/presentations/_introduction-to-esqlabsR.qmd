...
file_tree <- fs::dir_tree(esqlabsR:::projectDirectory("Example"), glob = "*.md|*.R", invert = TRUE)
```
...
Project's files are linked by the `ProjectConfiguration.json` file.
...
- targets `ProjectConfiguration.json` file,
...
my_project_configuration <-
  loadProject(path = "path/to/ProjectConfiguration.json")
```
...
```
ProjectConfiguration:
Project Configuration File: Example/ProjectConfiguration.xlsx
Model folder: Example/Models/Simulations
Configurations folder: Example/Configurations
Model parameters file: Example/Configurations/ModelParameters.xlsx
Individuals file: Example/Configurations/Individuals.xlsx
...
```
::: footer
I Project Configuration
...
**JSON as primary format** for version control and team collaboration

<br>

```{r, eval = FALSE}
# Migrate from Excel to JSON
...
```{r, echo = FALSE}
reactable::reactable(
  data = head(myScenarioResults$Aciclovir_iv$outputValues$data),
  columns = list(
...
Create plots (or plotgrids) from excel files using `createPlots()`.
