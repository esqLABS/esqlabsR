...
A `Project.json` container plus a `definitions/` tree, one JSON file per entity.
...
saveSnapshot(project, "aciclovir_study")  # writes aciclovir_study.esqlabsR

# Re-materialize it as a full project directory
loadSnapshot("aciclovir_study.esqlabsR", dir = "restored_project")
```
...
## Authoring is write-through
<br>

Every edit lands in its definition file immediately. There is no separate save
step.
...
sens <- sensitivityCalculation(
  ...
  outputPaths = project$outputPaths[["aciclovir_pvb"]],
