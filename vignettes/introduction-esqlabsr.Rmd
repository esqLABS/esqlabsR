...
The **`{esqlabsR}`** R-package is designed to facilitate and standardize **modeling and simulation** (M&S) of PBPK and QSP models implemented in [Open Systems Pharmacology Software](https://www.open-systems-pharmacology.org/) (OSPS) and executed from R. The package provides functions to read and run scenarios, workflows and simulations and create plots and visualization based on no-code input from Excel files. The package is based on R functions in the [`ospsuite` package](https://github.com/Open-Systems-Pharmacology/dev/OSPSuite-R).

## Motivation

This package builds up on the functionality of the `{ospsuite}` package. It is strongly recommended to get familiar with the [R Introduction](https://www.open-systems-pharmacology.org/OSPSuite-R/dev/). 
Functions reference for the `{ospsuite}` can be found [here](https://www.open-systems-pharmacology.org/OSPSuite-R/dev/reference/index.html), 
while articles describing the usage of the package with examples are found [here](https://www.open-systems-pharmacology.org/OSPSuite-R/dev/articles/).
...
* [Standard workflow](standard-workflow.html)
* [Data handling](data-handling.html)
* [Visualization and reporting](visualization-and-reporting.html)
* [Sensitivity analysis](sensitivity-analysis.html)
* [Available Shiny apps](shiny-apps.html)
...
* by default, the `Parameters` folder contains Excel files with global, population and individual model parameters, definitions of application protocols and modeling scenarios.

An exemplary project folder including the subfolders and files mentioned above can be found in the `/tests/data/TestProject` folder of the [`{esqlabsr}` repository](https://github.com/esqLABS/esqlabsR/tree/develop/tests/data/TestProject).
