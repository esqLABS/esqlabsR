...
configuration. Projects are now defined in a `ProjectConfiguration.json` file
...
Run `importProjectConfigurationFromExcel()` on your existing
`ProjectConfiguration.xlsx` to generate a `ProjectConfiguration.json` file:

```r
library(esqlabsR)
importProjectConfigurationFromExcel("path/to/ProjectConfiguration.xlsx")
```
...
`ProjectConfiguration.json` in the same directory. The Excel files are left
unchanged.
...
| `snapshotProjectConfiguration()` | `importProjectConfigurationFromExcel()` |
| `restoreProjectConfiguration()` | `exportProjectConfigurationToExcel()` |
