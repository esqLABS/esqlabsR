...
## Alternative: Adding scenarios programmatically

For more advanced users or when working programmatically, `esqlabsR` provides
the `addScenario()` function to add scenarios directly to a loaded project
configuration without manually editing the JSON file:

```{r, eval=FALSE}
# Load the project
project <- loadProject("Project.json")

# Add a new scenario programmatically
addScenario(
  project,
  scenarioName = "MyNewScenario",
...
The returned data frame contains the parameter paths, values, and units that
can be used to populate the `Applications.xlsx` file for your project.
...
folder defined in `Project$modelFolder`.
...
<!-- For demonstrating the simulation design process, we will use the example included in the package (which can be accessed using `exampleProjectPath`). -->
