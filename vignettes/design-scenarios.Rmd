...
simulation by defining them in the `ModelInitialValues.xlsx` file.
To apply initial conditions to a scenario, specify the name of the sheet
to load in the `InitialValuesSet` column of the `Scenarios.xlsx` file.
Multiple sheets can be specified as a comma-separated list (e.g.,
`GlobalInitial, CompoundInitial`).

The `ModelInitialValues.xlsx` file has one or more sheets, each
...
-   **Is Present**: A logical value (`TRUE`/`FALSE`) indicating whether
    the molecule is present. Rows where `Is Present` is `FALSE` are
    skipped.
...
The path to the `ModelInitialValues.xlsx` file is specified in the
project configuration under the `modelInitialValuesFile` property.
...
A list of sheet names from the `ModelInitialValues.xlsx` file, separated
...
to a molecule is constructed as `Container Path|Molecule Name`. Rows
where **Is Present** is `FALSE` are skipped. For every present molecule,
both **Value** and **Units** are mandatory; a missing value or an empty
unit raises an error. **InitialValuesSet** may be empty if no initial
...
Initial conditions defined via the **InitialValuesSet** column are
applied after all parameter values have been set, overriding the molecule
start values from the `.pkml` file.

### `ModelInitialValues.xlsx` file structure

The `ModelInitialValues.xlsx` file defines the initial conditions
(molecule start values) for simulations. The path to this file is defined
in `project$modelInitialValuesFile`. The file can contain
...
| `Is Present` | Logical (`TRUE`/`FALSE`). Rows with `FALSE` are skipped |
