...
| Output paths (`outputSelections`) | `outputPaths` | (none) |
...
```{r, eval=FALSE}
projectConfiguration <- createDefaultProjectConfiguration(path = "path/to/project")
...
Supply a **named vector** for `outputPaths` to assign short aliases to the
full model paths. The names become the `OutputPathId` values in `OutputPaths`
sheet of `Scenarios.xlsx`:
...
you can use a named vector for `outputPaths` where the names become the `OutputPathId`
values automatically — see [Custom output paths](#custom-output-paths) above.
