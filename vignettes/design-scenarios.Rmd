...
knitr::opts_chunk$set(
  eval = FALSE, # TODO(#908): re-enable when chapter 9 rewrites vignettes
  collapse = TRUE,
  comment = "#>",
  fig.showtext = TRUE
)
...
### 4a. Initial Conditions (Molecule Start Values)

You can override the initial conditions (molecule start values) of a
simulation by defining them in the `InitialConditions.xlsx` file.
To apply initial conditions to a scenario, specify the name of the sheet
to load in the `InitialConditions` column of the `Scenarios.xlsx` file.
Multiple sheets can be specified as a comma-separated list (e.g.,
`GlobalInitial, CompoundInitial`).

The `InitialConditions.xlsx` file has one or more sheets, each
containing molecule start values. Each sheet must have the following
columns:

-   **Container Path**: The path to the container holding the molecule
    (e.g., `Organism|Liver|Intracellular`).
-   **Molecule Name**: The name of the molecule (e.g., `DrugA`).
-   **Is Present**: A logical value (`TRUE`/`FALSE`) indicating whether
    the molecule is present. A numeric `0`/`1` cell is also accepted
    (`0` = absent, `1` = present), as Excel commonly stores logical
    columns as 0/1. Rows where `Is Present` is `FALSE` (or `0`) are
    skipped.
-   **Value**: The start value. Mandatory for present molecules; a
    missing or non-numeric value raises an error.
-   **Units**: The unit of the start value (e.g., `µmol`). Mandatory:
    an empty unit for a present molecule raises an error.
-   **Scale Divisor**: The scale divisor (for reference; not currently
    applied).
-   **Neg. Values Allowed**: Logical indicating if negative values are
    allowed (for reference; not currently applied).

The full path for each molecule is constructed as
`Container Path|Molecule Name`. Values are applied using
`ospsuite::setQuantityValuesByPath()` after all parameters are set.

The path to the `InitialConditions.xlsx` file is specified in the
project configuration under the `initialConditionsFile` property.
...
b)  To define individual-specific model parametrization, create one or
    more sheets in `Individuals.xlsx` and list their names (separated by
    a `,`) in the `Individual Parameter Sets` column of the
    `IndividualBiometrics` row for that individual. Parameters from the
    listed sheets are applied to the simulation in the order they appear
    in the column. Each subsequent sheet overrides parameters set by
    previous sheets. The `Individual Parameter Sets` column is required;
    leave it empty if no individual-specific model parameters are needed.

If your model contains proteins (e.g., enzymes or transporters), you
have to specify their ontogenies in the column 'Protein Ontogenies' as
a list (separated by a `,`) of `<Protein>:<Ontogeny>` values. The value
of `<Ontogeny>` must be one of the standard ontogenies implemented in
the PK-Sim database (call `ospsuite::StandardOntogeny` to see the list
of supported ontogenies). For example, if your model contains proteins
with names `CYP3A4_alternative` and `CYP2D6_alternative`, you can
specify their ontogenies by entering
...
`vignette("run-simulations")` to continue the process. To learn more
about simulation design, read the following sections.

## Alternative: Creating scenarios directly from PKML files

For more advanced users or when working programmatically, `esqlabsR` provides 
functions to create scenario configurations directly from PKML files, bypassing 
the need to manually fill in Excel files:

- `createScenarioConfigurationsFromPKML()`: Creates scenario configurations from 
  PKML files by automatically extracting simulation settings, applications, and
  output paths.
- `addScenarioConfigurationsToExcel()`: Writes those configurations to the 
  project's Excel files (`Scenarios.xlsx` and `Applications.xlsx`) so they can
  be used in the standard `esqlabsR` workflow.

This approach is particularly useful when:

- You have multiple PKML files to convert into scenarios at once.
- You want to automate or script scenario creation.
- Your simulation files already contain the settings you need (applications,
  output paths, simulation time).

### What is automatically extracted from the PKML file

When you call `createScenarioConfigurationsFromPKML()` without providing an
argument for a particular setting, the function reads it directly from the
PKML file:
...
You can override any of these by supplying the corresponding argument explicitly.
...
That's all. After calling `addScenarioConfigurationsToExcel()`, the scenario
appears in `Scenarios.xlsx` and any application protocol parameters are written
to `Applications.xlsx`, ready to be used with `createScenarios()`.
...
All arguments support vectorization. A **length-1** value is recycled to every
scenario, and **longer vectors** must match the total number of scenarios.
This makes it easy to create many scenarios from a single base model:
...
Supply a **named vector**, wrapped in a list, for `outputPaths` to assign
short aliases to the full model paths. The names become the `OutputPathId`
values in the `OutputPaths` sheet of `Scenarios.xlsx`:
...
By default, `addScenarioConfigurationsToExcel()` **appends** new scenarios to
the existing content of `Scenarios.xlsx`. Set `appendToExisting = FALSE` to
overwrite the file completely with only the new scenarios:
...
simulated individual. Individual-specific model parameters are defined
in sheets listed in the `Individual Parameter Sets` column of the
`IndividualBiometrics` row for that individual (see
Section [5. Individuals](#individuals)). **IndividualId** may be empty.
In this case, the individual as defined in the `pkml` simulation without
any individual-specific model parameters will be simulated. The same
...
A list of sheet names from the `InitialConditions.xlsx` file, separated
by a `,`. Each sheet must contain `Container Path`, `Molecule Name`,
`Is Present`, `Value`, `Units`, `Scale Divisor`, and
`Neg. Values Allowed`. Initial conditions from the specified sheets are
applied to the model after all parameter values have been set, overriding
the start values of molecules defined in the `.pkml` file. The full path
to a molecule is constructed as `Container Path|Molecule Name`. **Is
Present** accepts `TRUE`/`FALSE` or numeric `1`/`0`, and rows where it is
`FALSE` (or `0`) are skipped. For every present molecule,
both **Value** and **Units** are mandatory; a missing value or an empty
unit raises an error. **InitialConditions** may be empty if no initial
conditions need to be overridden.
...
If you need to add more parameters to an existing Excel file without overwriting it, 
you can use the `append` parameter:
...
-   for three weeks (equals to 3*7*24 = 504 hours) with a resolution of
    1 point per hour, and finally
-   for two days (equals to 504 + 2\*24 = 552 hours) with a resolution
    of 10 points per hour,
...
If `TRUE`, formula-defined parameters will be overwritten with their
steady-state values (corresponds to `ignoreIfFormula = FALSE` in
`ospsuite::getSteadyState()`). If empty or `FALSE` (default), only
constant parameters are updated with steady-state values and
formula-defined parameters are left unchanged (corresponds to
...
When creating scenarios programmatically with `createScenarioConfigurationsFromPKML()`,
you can provide `outputPaths` as a list of named vectors (one element per scenario),
where the element names become the `OutputPathId` values automatically — see
[Custom output paths](#custom-output-paths) above. Avoid passing a bare named vector,
which would be recycled across scenarios.
...
Initial conditions defined via the **InitialConditions** column are
applied after all parameter values have been set, overriding the molecule
start values from the `.pkml` file.

### `InitialConditions.xlsx` file structure

The `InitialConditions.xlsx` file defines the initial conditions
(molecule start values) for simulations. The path to this file is defined
in `project$initialConditionsFile`. The file can contain
multiple sheets, each representing a named "initial values set". Each
sheet must have the following columns:

| Column | Description |
|--------|-------------|
| `Container Path` | Path to the container holding the molecule (e.g., `Organism|Liver|Intracellular`) |
| `Molecule Name` | Name of the molecule (e.g., `DrugA`) |
| `Is Present` | Logical (`TRUE`/`FALSE`) or numeric (`1`/`0`). Rows with `FALSE`/`0` are skipped |
| `Value` | Start value of the molecule. Mandatory; missing/non-numeric values raise an error |
| `Units` | Unit of the start value (e.g., `µmol`). Mandatory; empty units for a present molecule raise an error |
| `Scale Divisor` | Scale divisor (for reference) |
| `Neg. Values Allowed` | Logical indicating if negative values are allowed (for reference) |

The full path of each molecule is constructed as
