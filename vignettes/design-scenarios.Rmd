...
| `modelParameters` | Keys in the top-level `modelParameters` map, applied in order |
...
`modelParameters` is an array of keys from the top-level
...
the groups to apply in their `modelParameters` array:
...
JSON field is `modelParameters`. The other `addScenario()` argument
...
| `ModelParameterSheets` | `modelParameters` |
...
`modelParameters` in JSON -- the underlying concept is the same (a named
...
`modelParameters`, groups are applied in order: later groups overwrite
earlier ones.
