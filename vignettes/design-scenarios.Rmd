...
-   **Units**: The unit of the start value (e.g., `µmol`). If empty,
    the value is assumed to be in base units.
...
where **Is Present** is `FALSE` are skipped. If **Units** is empty, the
value is assumed to be in base units. **InitialValuesSet** may be empty
if no initial conditions need to be overridden.
...
| `Value` | Start value of the molecule |
| `Units` | Unit of the start value (e.g., `µmol`). Empty means base units |
