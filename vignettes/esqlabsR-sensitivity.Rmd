...
kind of numerical model outputs using user-defined functions. As for the
PK parameters, the provided function(s) will be applied to each of the
output defined in the argument `outputPaths` of the
...
`xValues` and `yValues`, through it accesses the simulated time values
(`xValues`) or the output values (`yValues`). The function should return
...
[**Note**]{.underline}: If we want to calculate the sensitivity of the custom function
only without the default PK-paraeters, we need to set the `pkParameters` value 
to an empty list.
