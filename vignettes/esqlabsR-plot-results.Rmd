...
`{esqlabsR}` implements an excel-based workflow for creation of figures from simulated 
scenarios. The figures are defined in the excel file "Plotx.xlsx". The general workflow is:

CREATE A WORKFLOW FIGURE! (multiple) simulation results coming from simulated 
scenarios plus (multiple) observed data sets are added to `DataCombined` (sheet `DataCombined`).
...
Describe the most important columns, namely those present in the Plots.xlsx of 
the example project. Describe that the user can additionally add a column for any 
property of the respective `configuration` class (example for this).
...
!!! MOVE THE SECTION BELOW TO SOMETHING LIKE "basics of figure creation with ospsuite-r".
