...
and quality control. `{esqlabsR}` implements an excel-based workflow for figure
creation directly from simulated scenarios.
...
translate the information in the excel file into plots ! The workflow that 
enables this is described in the [Plotting Workflow](#plotting-workflow) section below. 
It also remain possible to generate plot from simulation result objects while 
using the esqlabsR default theme. This, is described in the [Generate Single Plot](#generate-single-plot) section.
...
the **plotConfiguration** sheet. More information in the 
[Customizing Plots](#customizing-plots) section.

All plots can be combined using their IDs into multi-panel figures (plot grids) 
in the **plotGrids** sheet. More information in the [Combine Plots](#combine-plots) section.

Plot grids can be exported with specific settings using the 
**exportConfiguration** sheet. More information in the [Export Plot](#export-plots) section.

Finally, calling the `createPlotsFromExcel()` function will create the plots and
export them to image files if specified in the **exportConfiguration** sheet. 
...
(set dataType to `observed`) and give them labels. For Simulated results, 
the user needs to specify the name of the scenario and the output path of the 
...
Other columns available in this sheet are relative to the data transformations that
can be performed by `dataCombined`. For more information on this, please refer to
the `{ospsuite}` documentation about it [here](https://www.open-systems-pharmacology.org/OSPSuite-R/articles/data-combined.html#transformations).
...
<!-- From a single `DataCombined`, multiple plots (individual/population time profile,  -->
<!-- simulated-vs-observed, etc) can be created in `plotConfiguration` sheet. The plots -->
<!-- with ther plot IDs can be combined to multi-panel figures in the sheet `plotGrids`. -->
<!-- THIS IS THEN THE FINAL PLOT THAT WILL BE RETURNED. So even for a plot with only one  -->
<!-- panel, a plot grid must be defined. -->

<!-- Calling function `createPlotsFromExcel()` returns ggplot-objects, one object for  -->
<!-- each plot grid (all or only specified). By default, if any of the simulated results  -->
<!-- or observed data specified for the plots is not found, and error is thrown. If setting -->
<!-- `ignoreIfNotFound = FALSE`, these data are simply ignored and not added to the plots. -->
<!-- Might be useful if not all scenarios are simulated, but still want to plot at least -->
<!-- the data. -->

<!-- IF you want to save plots to png during creation, list the plot grids in the  -->
<!-- sheet `exportConfiguration`. -->



<!-- For properties that accept a list of values (e.g. `xAxisLimits`), the values are  -->
<!-- separated by a `,`. If an entry itself should contain a `,`, e.g. the name of a plotID -->
<!-- that is combined to a multi-pannel plot in a plot grid, enclose the entry into parenthesis. -->

<!-- E.g. in column `plotIDs` of the `plotGrids` sheet:  -->

<!-- `"Name of the first plot, which contains a comma", "Name of the secold plot"`. -->

<!-- You can also return the `DataCombined` objects defined in the `DataCombined` sheet -->
<!-- with the function `createDataCombinedFromExcel()`. -->


## Generate Single Plot

In some situation, the user needs to quickly draw a plot from a simulation result
object while wanting to use the default esqlabsR theme.
...
Simulated modeling scenarios can be passed to
...
described in `vignette("esqlabsR-design-scenarios")` and load the
corresponding observed data as described in the [Observed Data](#observed-data) section.
...
```{r plot-time-profile, fig.width=7, fig.height=7, eval = FALSE}
...
By default, the height of the output figure is calculated from the number of rows 
in the multi-panel plot and the height defined in `ExportConfiguration$heightPerRow`.
If you want to define a fixed height with the parameter `ExportConfiguration$height`,
