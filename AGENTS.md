...
- All files in `R/` use **kebab-case**: `project-lifecycle.R`, `scenario-execution.R`, `observed-data.R`.
- Files are organized by domain/functionality, not by class hierarchy.
- Helper collections use a `-utils.R` suffix: `data-utils.R`, `file-utils.R`, `plots-utils.R`.
- Tests mirror their source file: `R/project-lifecycle.R` → `tests/testthat/test-project-lifecycle.R`.
