...
- **`NEWS.md` compares against the last release, not against yesterday's branch.** Its reader is a user upgrading from the latest released version, so every bullet states what the development version does, measured against that release. A change that only moves one unreleased state to another is invisible to that reader and gets no bullet: never write "now X instead of Y" when Y never shipped, and never add a bullet for fixing, renaming, or reshaping something this same unreleased cycle introduced. When such a change makes an existing bullet inaccurate, edit that bullet so it describes the current development state; when an existing bullet already covers the feature, add nothing. The development history itself belongs in `REFACTORING-LOG.md`, the commit, and the PR.
...
- **Ids are canonicalized; shared vectorization helpers.** Run ids through `.canonicalizeId()`, and use the shared `.assertIdVector()` / `.assertNoDuplicateIds()` / `.recycleField()` / `.alignAuthoringArgs()` helpers rather than re-implementing per domain. An id is canonicalized because it names a definition file; an id that names no file is not. The one such id in the tree is a PI member id (a `PIParameter` / `PIOutputMapping` `id`), which labels a record inside its task's file: every door stores it exactly as given, and every lookup matches it exactly. Canonicalizing one door alone would make a mixed-case member unreachable, since nothing canonicalizes to a string containing an uppercase letter.
- **User-facing text goes through the `messages` catalog.** Add an entry in `R/messages.R` and raise it via `cli::cli_abort` / `cli::cli_warn` / `cli::cli_inform`. Don't `stop()` / `warning()` on inline literals. Build the text with `cliFormat()` by default, with `cli::format_message()` when the message carries `"i"` / `"x"` / `"!"` / `"*"` bullets (the one thing `cliFormat()` can't express, since it formats a single inline string and drops the names of a `cli` vector), and with the raw unglued template when a value interpolated into the message may itself contain `{` or `}` (both helpers glue eagerly, and the raising call glue-parses the result a second time). The header of `R/messages.R` states this in full. (Known exception: project validators build `validationResult` messages inline with `paste0()`.)
