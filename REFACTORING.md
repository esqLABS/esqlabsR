...
- **A project has one on-disk layout, whoever made it.** The working folders are `Models/Simulations`, `Models/Snapshots`, `Data`, `Populations`, `Results/Figures`, `Results/SimulationResults`, each carrying a `README.md` so git tracks it; `definitions/` holds the authored definitions and the container is `Project.json`. The values live in `.nominalProjectFolders` (`R/project-excel.R`), the scaffold in `.scaffoldProjectFolders()` (`R/project-lifecycle.R`), and the `Blank` / `Example` templates must agree with both. Every entry point that produces a project (`initProject()`, `importProjectFromExcel()`, `restoreProject()`) emits that structure: a migration path normalizes the old project's folder values and copies its content into the nominal folders rather than preserving the legacy names. The one exception is a folder the project deliberately places outside itself, as an absolute path or a `${VAR}`, which keeps its value.
