...
| B10 | The "PI task not found" lookup-and-abort is written out in all four PI authoring impls, and the whole 11-line removal body (drop the member, then drop the task if it is now empty) twice more, all with inline message literals rather than catalog entries. | `parameter-identification.R`, four lookup sites plus the two removal impls | duplicated knowledge | low | S | P2 | fixed |
...
| B19 | `plotIndividualTimeProfile()` is deprecated in ospsuite 12.4.2 and slated for removal in ospsuite 14.0. The suggested replacement `plotTimeProfile()` takes plot data plus a ggplot2 mapping rather than a `DataCombined` plus a plot configuration, so it is not a drop-in. `plotPopulationTimeProfile()` is unaffected. | `create-plots.R:578` | dated dependency | medium (breaks outright on ospsuite 14.0) | M | P2 | open |
| B20 | `ospsuite.utils::validateEnumValue()` / `enumGetKey()` compare a value against an enum with `==` without matching lengths first, so nine base-R warnings (`NAs introduced by coercion`, `longer object length is not a multiple of shorter object length`) surface through the sensitivity unit validation. | reached from `sensitivity-time-profiles.R:468,612` | upstream defect | low | S | P3 | open (upstream) |
