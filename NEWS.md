...
- `importProjectFromExcel(projectConfigPath, outputDir, silent, overwrite)` reads an Excel-based project and writes a v2.0 `Project.json`; `exportProjectToExcel(project, outputDir, silent)` writes the reverse. The bridge is a lossless round trip: every project section, including model/individual/application parameter sets, scenarios, the nested `dataCombined` plots, observed data, and parameter identification, survives Excel and back. Parameter-set names that exceed Excel's 31-character sheet-name limit or contain forbidden characters are preserved through sanitisation, so scenario references are never orphaned. `exportProjectToExcel()` writes into the project's configured `configurationsFolder` and clears stale section workbooks so a removed section does not reappear; `importProjectFromExcel()` refuses to overwrite an existing `Project.json` unless `overwrite = TRUE`. `projectStatus(project)` (and `project$sync()`) report synchronisation between the in-memory project, the JSON file, and the Excel side-cars. (#908, #1049)
