...
- `readInitialValuesFromXLS()` is added to read molecule start values from a structured Excel file (`Container Path`, `Molecule Name`, `Is Present`, `Value`, `Units`, `Scale Divisor`, `Neg. Values Allowed`). A new `InitialValuesSet` column in `Scenarios.xlsx` references sheets in a new `ModelInitialValues.xlsx` file; existing `Scenarios.xlsx` files must add this column (it can be left empty). A new `modelInitialValuesFile` path field is added to `Project`. Values are applied during simulation initialization using `ospsuite::setQuantityValuesByPath`. (#973)
