...
- `isProjectInitialized()` now answers the JSON-first question: it reports `TRUE` only when the folder holds a `Project.json`. A folder holding nothing but a stray `SomeProjectData.xlsx`, or a leftover empty `Configurations/` folder, was previously reported as an initialized project, since the Excel files were still treated as project markers. The overwrite guard that check also served has moved into `initProject()` itself, so `initProject()` still asks before scaffolding over an unmigrated pre-6.0.0 Excel project. (#1194)
