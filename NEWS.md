...
- `saveProject()` now records the esqlabsR version that wrote the file. A project's `esqlabsRVersion` was carried over from the container it was loaded from, so a project authored under 6.0.0 still reported `6.0.0` after a later version had saved it. The field now names the version that last wrote the project, matching what `importProjectFromExcel()` and `exportProjectToExcel()` already stamp; `schemaVersion` is likewise written as the format the writer produces. `snapshotProject()` writes both fields the same way, since it shares the serializer.
