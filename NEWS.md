...
- The full programmatic mutation API: `addScenario()` / `removeScenario()`, `addIndividual()` / `removeIndividual()`, `addPopulation()` / `removePopulation()`, `addApplication()` / `removeApplication()`, `addOutputPath()` / `removeOutputPath()`, the parameter-set helpers, `addObservedData()` / `removeObservedData()`, the plot helpers, and the PI helpers `addPITask()` / `removePITask()` (plus inline `addPIParameter()` / `addPIOutputMapping()` and their removals). (#908)
...
- `createScenariosFromPKML(pkmlFilePaths, project, ...)` reads scenarios from PKML files into a `Project`. Steady-state overwrite of formula-defined parameters is controlled by the `overwriteFormulasInSS` field on a `Scenario` (corresponding to `ignoreIfFormula = FALSE` in `ospsuite::getSteadyState()`), defaulting to `FALSE`. (#908)
