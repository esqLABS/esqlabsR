...
- `runScenarios()` gains a `stopIfFails` argument that defaults to `TRUE`, so a scenario whose simulation produces no results now aborts the run by default instead of warning and returning the other scenarios. Set `stopIfFails = FALSE` to restore the previous warn-and-continue behaviour. (#1036)
