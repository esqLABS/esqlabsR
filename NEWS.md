...
- `Project` section fields (`scenarios`, `outputPaths`, `individuals`, `populations`, the parameter sets, `observedData`, `plots`, `parameterIdentification`) are now active bindings that mark the project modified on any write, including subscript and nested assignment such as `project$scenarios[["X"]] <- sc` or `project$scenarios <- c(project$scenarios, more)`. `Scenario` is now a plain-data record built with `Scenario()` (was `Scenario$new()`) with copy semantics, so an entry read from `project$scenarios` is an independent copy and must be written back to take effect. `project$jsonPath` is read-only. (#1048)
