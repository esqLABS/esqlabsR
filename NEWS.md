...
- A project-controlled file path that resolves outside the project folder is now rejected. A scenario's `modelFile`, a population id (`.csv`), and an observed-data source's `file` / `importerConfiguration` are each required to stay under their working folder, so a shared or downloaded project can no longer read a file elsewhere on the machine via a `../` path; the loader aborts naming the offending field instead of failing later with a misleading "file not found". (#1034)
