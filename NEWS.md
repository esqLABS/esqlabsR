...
- `readInitialConditionsFromXLS()` is added to read molecule start values from a structured Excel file (`Container Path`, `Molecule Name`, `Is Present`, `Value`, `Units`, `Scale Divisor`, `Neg. Values Allowed`); `Is Present` accepts `TRUE`/`FALSE` or numeric `1`/`0`, and units are mandatory for every present molecule. A new `InitialConditions` column in `Scenarios.xlsx` references sheets in a new `InitialConditions.xlsx` file; existing `Scenarios.xlsx` files must add this column (it can be left empty). A new `initialConditionsFile` path field is added to `Project`, `addScenario()` gains an `initialConditions` argument referencing `project$initialConditions`, and `validateProject()` reports scenarios that reference an undefined initial-conditions set. Values are applied during simulation initialization using `ospsuite::setQuantityValuesByPath`. (#973)
