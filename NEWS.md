...
- `PIOutputMapping()` (and the `outputPath` argument of `addPITask()` / `addPIOutputMapping()`) now accepts either an output-path id or the literal model path of a defined output path, resolving both to the id; an `outputPath` matching no defined output path aborts with a "did you mean" hint. (#1157)
...
- `runScenarios()` and `buildSimulations()` now match the `scenarios` argument case-insensitively against the canonical scenario ids, so a scenario runs under the mixed-case name it was authored with. (#1157)
