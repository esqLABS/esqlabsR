...
- `loadProject(path)` loads a `Project` and is the primary entry point for the workflow. `path` is either the `Project.json` or the project folder holding it, so the folder an import or a restore just produced opens directly (a project file saved under another name still has to be named in full, and a folder with no `Project.json` says so instead of failing with a JSON parse error). Pointed at a project snapshot written by a previous esqlabsR version, it says the file is a previous-version snapshot and names `restoreProject()`, which upgrades one, instead of reporting an unsupported schema version. (#908, #1174, #1185)
...
- `importProjectFromExcel()` now produces the same project structure `initProject()` creates, whatever layout the Excel project used, so a project has one shape however it was made. It copies the folders the project references (the simulations folder, the data folder, and the populations folder) into the new project and gives it the standard working-folder scaffold (`Models/Simulations`, `Models/Snapshots`, `Data`, `Populations`, `Results/Figures`, `Results/SimulationResults`, each with a short `README.md`), so the imported project is runnable where it was written. Previously only the definition tree was written, and because a definition references a model or data file by a path relative to the project folder, every such reference dangled: the import ended in File-Not-Found warnings until the models and data were hand-copied. `filePaths` carries the standard folder values rather than the Excel project's own spellings, and the content is copied to match: population csvs kept where 5.x projects keep them (`Configurations/PopulationsCSV/`, which the import previously looked for at the project root and reported as absent, leaving every `readPopulationFromCSV` scenario to fail at run time) now land in `Populations/`. Whole folders travel, so an asset nothing names statically (a data-importer configuration, a population csv) comes along too, and a folder that was renamed on the way in is reported as `old -> new`. Nothing is moved or deleted, so an import in place leaves the Excel project working beside the imported one. Not copied: the output folder (results the project writes, not inputs it reads), a folder given as an absolute path or through a `${VAR}` (it deliberately sits outside the project, keeps its value, and resolves from anywhere), a `../`-climbing folder value naming a location the project does not own, and a target folder that already holds files unless `overwrite = TRUE` (so a model or data file you curated in the output beforehand is never silently replaced). A folder that was not copied is reported. The `copyAssets` argument (default `TRUE`) turns the content copy off when only the definitions are wanted; the project still gets its standard structure. The non-folder `dataFile` and `dataImporterConfigurationFile` properties no longer leak into `filePaths`. (#1132, #1164)
