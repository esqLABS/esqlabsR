...
- `readInitialConditionsFromXLS()` is added to read molecule start values from a structured Excel file (`Container Path`, `Molecule Name`, `Is Present`, `Value`, `Units`, `Scale Divisor`, `Neg. Values Allowed`). A new `InitialConditions` column in `Scenarios.xlsx` references sheets in a new `InitialConditions.xlsx` file; existing `Scenarios.xlsx` files must add this column (it can be left empty). A new `initialConditionsFile` path field is added to `Project`. Values are applied during simulation initialization using `ospsuite::setQuantityValuesByPath`. (#973)
